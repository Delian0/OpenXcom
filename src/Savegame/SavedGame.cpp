...
void SavedGame::load(const std::string &filename, Mod *mod, Language *lang)
{
	...
	{
		std::string researchName = discovery.readVal<std::string>();
		if (RuleResearch* research = mod->getResearch(researchName))
		{
 ...
 */
bool SavedGame::handleResearchUnlockedByMissions(const RuleResearch* research, const AlienDeployment* deployment, const Mod* mod)
{
	if (!research)
	{
		return false;
	...
	{
		RuleResearch* lookup = mod->getResearch(research->getLookup(), true);
		if (!isResearched(lookup, false))
			addResearchDiaryEntryForMission(lookup, DiscoverySourceType::MISSION, deployment, nullptr);
		researchVec.push_back(lookup);
	...
	if (auto* bonus = selectGetOneFree(research))
	{
		...
		if (!bonus->getLookup().empty())
		{
			...
			if (!isResearched(bonusLookup, false))
				addResearchDiaryEntryForMission(bonusLookup, DiscoverySourceType::FREE_FROM, nullptr, research);
			researchVec.push_back(bonusLookup);
			addFinishedResearch(researchVec.back(), mod, base, true);
		}
	}
